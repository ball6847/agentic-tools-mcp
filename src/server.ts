 ...
 */
export async function createServer(config: StorageConfig = defaultStorageConfig): Promise<McpServer> {
  ...
  if (!config.disableTaskManagement) {
    server.tool(
      'list_projects',
...
• Add more nested tasks with \`create_task\` using parentId`
  ...
  if (!config.disableMemoryManagement) {
    server.tool(
      'create_memory',
